...
shared_ptr<Object> TransferTransactionParser::parse(string data)
{
    ...
    // Ít nhất cần srcAcc, destAcc, Amount, PIN
    if (v.size() >= 4)
    {
        ...
        shared_ptr<Object> object = make_shared<TransferTransaction>(bank->getAccount(srcAccNum), bank->getAccount(destAccNum), amount, PIN);
        return object;
    }
    return nullptr;
}